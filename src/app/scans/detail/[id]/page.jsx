...
export default function DetailScan() {
  ...
  useEffect(() => {
    ...
    const fillDataDetialed = async () => {
      try {
        const { data } = await api.get(`/scans/${param.id}`);
                ...
                <div className="bg-white rounded-lg p-8 @max-[768px]:flex-row md:flex gap-5">
                  ...
                  <div className="flex-col gap-3 text-[15pt] mt-2">
                    ...
                    <h3 className="font-bold text-[25pt] text-blue-800 mb-4">
                      Gambar oleh {dataDetailed.user.name}
                    </h3>
                    <p className="flex gap-2 mb-2">
                      ...
                      <span>{dataDetailed.predicted}</span>
                    </p>
                    <p className="flex gap-2">
                      <span className="flex w-[170px] text-gray-600 font-light">
                        <span className="flex-grow-1">Diupload pada</span>
                        <span>:</span>
                      </span>
                      <span>{dataDetailed.created_at.split("T")[0]}</span>
                    </p>
                  </div>
                  <div className=""></div>
                </div>
